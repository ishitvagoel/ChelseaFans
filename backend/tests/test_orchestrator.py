...
def _cache_key() -> str:
    return f"chelsea:just-finished:{JUST_FINISHED_CACHE_VERSION}"
...
async def test_empty_fixture_result_is_not_snapshotted() -> None:
    ...
    assert result == []
    assert await snaps.get(_cache_key()) is None


@pytest.mark.asyncio
async def test_just_finished_persists_more_matches_than_request_limit() -> None:
    demo = DemoProvider()
    matches = await demo.recent_finished(team_hint="Chelsea", limit=8)
    ...
    cache = InMemoryCache()
    orch = ProviderOrchestrator(registry, cache, NullSnapshotRepository())
    result = await orch.just_finished(4)
    assert len(result) == 4
    cached = await cache.get_json(_cache_key())
    assert isinstance(cached, list)
    assert len(cached) == len(matches)
    later = await orch.just_finished(6)
    assert len(later) == 6
    assert counter.calls == 1
